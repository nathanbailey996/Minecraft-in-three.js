...
export default class Terrain {
    constructor(_blockTypes){
        ...
        this.setTerrainInfo()
        const previousWorld = this.getFromLocalStorage()
        if(previousWorld){
          console.log(previousWorld)
          this.terrain.arrayOfChunks = previousWorld.blockPositions
          this.getBoundrys(previousWorld.blockPositions, true)
          this.displayBlocks(previousWorld.blockPositions, true)
          if(this.experience.world.addingRemovingBlocks){
            this.experience.world.addingRemovingBlocks.infiniteDepthBlocks = previousWorld.infiniteDepthBlocks
            this.experience.world.addingRemovingBlocks.addedBlocks = previousWorld.addedBlocks
            this.experience.world.addingRemovingBlocks.removedBlocks = previousWorld.removedBlocks
            // this.experience.world.addingRemovingBlocks.originalBlocks = previousWorld.originalBlocks
          }
        }
        else{
        this.setTerrain()   
    ...
    getFromLocalStorage(){
     return JSON.parse(localStorage.getItem('arrayOfChunks'))
    }
    ...
    setTerrain(){
        ...
        //     this.terrain.distanceFromEdge =( this.terrain.numberOfChunks * this.terrain.numberOfChunks * this.terrain.chunkSize) * 0.3
            ...
            this.blockTypes.blocks.grass.count = this.terrain.chunkSize * this.terrain.chunkSize *this.terrain.numberOfChunks * this.terrain.numberOfChunks
            this.blockTypes.blocks.grass.instancedMesh = new THREE.InstancedMesh(this.blockTypes.blocks.grass.geometry, this.blockTypes.blocks.grass.material, this.blockTypes.blocks.grass.count)
        ...
        this.blockTypes.blocks.sandBlock.instancedMesh = new THREE.InstancedMesh(this.blockTypes.blocks.sandBlock.geometry, this.blockTypes.blocks.sandBlock.material, this.blockTypes.blocks.sandBlock.count)
  ...
  //sorts the blocks in terms of their depth and creates the instanced mesh for each block type needed to regenerate
    displayBlocks(_arrayOfChunks, _removePreviousMesh){
      const blockData = {
        blockPositions:_arrayOfChunks, 
        infiniteDepthBlocks:this.experience.world.addingRemovingBlocks? this.experience.world.addingRemovingBlocks.infiniteDepthBlocks: [] , 
        addedBlocks:this.experience.world.addingRemovingBlocks? this.experience.world.addingRemovingBlocks.addedBlocks: [], 
        removedBlocks:this.experience.world.addingRemovingBlocks? this.experience.world.addingRemovingBlocks.removedBlocks:[],
        // originalBlocks:this.experience.world.addingRemovingBlocks? this.experience.world.addingRemovingBlocks.originalBlocks:[],
      }
      

      localStorage.setItem('arrayOfChunks', JSON.stringify(blockData))
              ...
              case -1:
                ...
                switch(_blockPosition.type){
                  case 1:
                    ...
                    break;
                  // case 2:
                  //   newWaterChunk.push(_blockPosition)
                  //   break
                  case 3:
                    newStoneChunk.push(_blockPosition)
                  case 4:
   ...
   this.blockTypes.blocks.grass.count)
    ...
    update(){
      // console.log(this.experience.camera.instance.position.z , this.terrain.boundrys.smallestZ +  this.terrain.distanceFromEdge)
      // console.log(this.terrain.boundrys.smallestZ)
      if(this.experience.camera.instance.position.z <= this.terrain.boundrys.smallestZ +  this.terrain.distanceFromEdge){
        ...
        for(let i = 0; i< this.terrain.arrayOfChunks.length; i++){
         ...
         for(let i = 0; i< this.terrain.numberOfChunks; i++){
          ...
          //  arrayOfIndexes.push(i * this.terrain.numberOfChunks)
          newPositionsArray.splice(i * this.terrain.numberOfChunks , 0, newChunk)
         }

         this.terrain.arrayOfChunks = newPositionsArray
        //  const arrayToGetIndexes = []
        //  for(const _arrayIndex of arrayOfIndexes){
        //   arrayToGetIndexes.push(...newPositionsArray[_arrayIndex])
        //  }
        // this.
